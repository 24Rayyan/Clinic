@extends('welcome')
@section('tittle')
    Clinic | Antrian Spesialis
@endsection
@section('content')
<div class="container mt-5">
    <h1 class="text-center mb-4">Antrian Poli Spesialis<sup><img src="{{asset('addon/hospital.png')}}" style="height: 50px"  alt=""></sup></h1>
    ...
    <div class="mb-4 d-flex justify-content-end">
        <form action="{{ route('resetQueue', 'spesialis') }}" method="POST" class="mr-2">
            @csrf
            <button type="submit" class="btn btn-danger">Reset Nomor Antrian</button>
        </form>
    
        <form action="{{ route('restoreSpesialis') }}" method="POST">
            @csrf
            @method('DELETE')
